...
const DialogItem: FC<PropsType> = ({user, message, isChecked, isMe}) => {
    ...
    const getAvatar = (avatar: string | undefined) => {
        const random = user._id > 9 ? 14 - Number(user._id.toString().slice(-1))
            : user._id = (14 - user._id);
        if (avatar) {
            return <img
                src={avatar}
                alt={user.fullName}/>;
            ...
            return <>
                <div className='dialogs__item-noAvatar'
                     style={{backgroundColor: AvatarColors[random]}}>
